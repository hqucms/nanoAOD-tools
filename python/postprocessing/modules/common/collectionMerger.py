...
class collectionMerger(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        branches_in = set([_brlist_in.At(i) for i in range(_brlist_in.GetEntries())])
        branches_in = [x for x in branches_in if inputTree.GetBranchStatus(x.GetName())]

        # Find list of activated branches in output tree
        _brlist_out = wrappedOutputTree._tree.GetListOfBranches()
        branches_out = set([_brlist_out.At(i) for i in range(_brlist_out.GetEntries())])
        branches_out = [x for x in branches_out if wrappedOutputTree._tree.GetBranchStatus(x.GetName())]
    ...
    def analyze(self, event):
        ...
        if self.selector:
            objects = [obj_j_i for obj_j_i in objects if self.selector[obj_j_i[1]](obj_j_i[0])]
...
def lepMerger(): return collectionMerger(input=["Electron", "Muon"], output="Lepton")


def lepMerger_exampleSelection(): return collectionMerger(input=["Electron", "Muon"], output="Lepton",  # this will keep only the two leading leptons among electrons with pt > 20 and muons with pt > 40
                                                          maxObjects=2,
