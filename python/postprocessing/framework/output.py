...
class OutputBranch:
    ...
    def fill(self, val):
        if self.lenVar:
            ...
            if len(val) != self.n:
                raise RuntimeError("Mismatch in filling branch %s of fixed length %d with %d values (%s)" % (self.Branch.GetName(), self.n, len(val), val))
            for i, v in enumerate(val):
                self.buff[i] = v
    ...
    def __init__(
        ...
        for k in inputFile.GetListOfKeys():
            ...
            if kn == "Events":
                ...
                if provenance:
                    self._otherTrees[kn] = inputFile.Get(kn).CopyTree('1' if firstEntry == 0 else '0')  # treat content of other trees as if associated to event 0
            elif kn in ("LuminosityBlocks", "Runs"):
                if not jsonFilter:
                    self._otherTrees[kn] = inputFile.Get(kn).CopyTree('1' if firstEntry == 0 else '0')
                elif firstEntry == 0:
                    _isRun = (kn == "Runs")
